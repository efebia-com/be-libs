...
describe("routeV4", () => {
  ...
  it("should throw an error if returning 200 but it is not in response schema", async () => {
    ...
    assert.deepStrictEqual(response.json(), {
      ...
      message:
        "Reply schema of: / does not have the specified status code: 201.",
    });
    assert.equal(
      response.headers["content-type"],
      "application/json; charset=utf-8"
  ...
  describe("strict", () => {
    ...
    it("strict true local mode should add additionalProperties: false", () => {
      ...
      assert.equal(
        (value.schema?.querystring as any).additionalProperties,
        false
      );
    });
    it("strict false local mode should not add additionalProperties", () => {
      ...
      assert.equal(
        (value.schema?.params as any).additionalProperties,
        undefined
      );
      assert.equal(
        (value.schema?.querystring as any).additionalProperties,
    ...
    for (const t of [true, false]) {
      it(`local option should overcome global options (global: ${t}, local: ${!t})`, () => {
        ...
        assert.equal(
          (value.schema?.body as any).additionalProperties,
          t ? undefined : false
        );
        assert.equal(
          (value.schema?.params as any).additionalProperties,
