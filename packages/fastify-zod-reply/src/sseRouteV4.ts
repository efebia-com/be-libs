...
// The mapped type to transform the key
type TransformSSETo200<T> = {
	[K in keyof T as K extends 'SSE' ? 200 : K]: T[K];
};

// The updated FastifySSEZodV4Schema
...
export function createSSERouteV4(globalOptions: SSERouteV4Options = {}) {
	return <
	...
	>(
		...
		return {
			...
			preHandler: async (request, reply) => {
				...
				reply.sse = async <T extends SSEReplyShape>(options: {
